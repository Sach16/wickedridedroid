...
public class RequestManager<T> {
    ...
    //public static final String TESTING_SERVER = "http://wickedride.com/api/v1/";
    //public static final String TESTING_SERVER = "http://54.255.177.26/api/";

    public static final String TESTING_SERVER = "http://54.254.150.67/api/";

    public static final String LIVE_SERVER = "https://wickedride.com/api/";
    ...
    public void placeRequest(String methodName, Class<T> clazz, ServerCallback<T> listener, HashMap<String, String> feedParams, boolean isPOST) {
        ...
        if (SessionManager.isUserSessionValid(mCtx) && (methodName.equals(APIMethods.PROFILE_DETAILS) || methodName.contains(APIMethods.REVIEWS))) {
            HashMap<String, String> headers = new HashMap<String, String>();
            headers.put(FeedParams.HEADERS_AUTHENTICATION, "Bearer " + SessionManager.getSessionToken(mCtx));
     ...
     * Method to create the request from the passed parameters and place a Volley request
     */
    public void placeRequest(String methodName, Class<T> clazz, ServerCallback<T> listener, boolean isPost ,JSONObject jsonObj) {

        String feedurl = baseFeedURL.concat(methodName);

        Log.d(LOG_TAG, "SearchURL::" + feedurl);
        JSONRequest<T> jsObjRequest = new JSONRequest<T>(isPost ? Method.POST : Method.GET,
                feedurl,
                methodName,
                clazz,
                jsonObj,
                ...
                createSuccessListener(),
                createErrorListener());
        if (SessionManager.isUserSessionValid(mCtx) && methodName.equals(APIMethods.PROFILE_DETAILS)) {
            HashMap<String, String> headers = new HashMap<String, String>();
            headers.put(FeedParams.HEADERS_AUTHENTICATION, "Bearer " + SessionManager.getSessionToken(mCtx));
            jsObjRequest.setHeaders(headers);
        }
     ...
     * Method to create the request from the passed parameters and place a Volley request
     */
    public void placeRequest(String methodName, Class<T> clazz, ServerCallback<T> listener, HashMap<String, String> feedParams,boolean isPOST,boolean addTokenHeader,boolean isPut) {
        ...
        GsonRequest<T> jsObjRequest = new GsonRequest<T>(method,
                feedurl,
                methodName,
                clazz,
                feedParams,
        ...
        if (SessionManager.isUserSessionValid(mCtx) && addTokenHeader) {
            HashMap<String, String> headers = new HashMap<String, String>();
            headers.put(FeedParams.HEADERS_AUTHENTICATION, "Bearer " + SessionManager.getSessionToken(mCtx));
            jsObjRequest.setHeaders(headers);
        }
...
//		}
        ...
        if (SessionManager.isUserSessionValid(mCtx)) {
            HashMap<String, String> headers = new HashMap<String, String>();
            headers.put(FeedParams.HEADERS_AUTHENTICATION, "Bearer " + SessionManager.getSessionToken(mCtx));
            // Add  "device" holding value "MOBILE"
            jsObjRequest.setHeaders(headers);
        }
